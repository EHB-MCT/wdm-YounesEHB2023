...
export default function AdminDashboard() {
	...
	const fetchUserDetailData = async (userId) => {
		try {
			const [activityResponse, recordsResponse, summaryResponse] =
				await Promise.all([
					fetch(
						`http://localhost:5000/api/admin/users/${userId}/activity-timeline?limit=5`,
						{
							headers: {
					...
					fetch(
						`http://localhost:5000/api/admin/users/${userId}/personal-records`,
						{
							headers: {
								Authorization: `Bearer ${adminToken}`,
							},
					...
					fetch(
						`http://localhost:5000/api/admin/users/${userId}/workout-summary`,
						{
							headers: {
								Authorization: `Bearer ${adminToken}`,
							},
	...
	const fetchAdminData = useCallback(async () => {
		try {
			const [
				...
				fetch(
					`http://localhost:5000/api/admin/charts/user-growth?period=${chartPeriod}`,
					{
						headers: {
							Authorization: `Bearer ${adminToken}`,
						},
				...
				fetch(
					`http://localhost:5000/api/admin/charts/workout-frequency?period=${chartPeriod}`,
					{
						headers: {
							Authorization: `Bearer ${adminToken}`,
						},
				...
				fetch(
					`http://localhost:5000/api/admin/charts/activity-heatmap?period=${chartPeriod}`,
					{
						headers: {
							Authorization: `Bearer ${adminToken}`,
						},
			...
			if (userTypeDistributionResponse.ok) {
				const userTypeDistributionData =
					await userTypeDistributionResponse.json();
				setChartData((prev) => ({
					...prev,
					userTypeDistribution: userTypeDistributionData,
				}));
			}
		} catch {
	...
	const getFilteredUsers = () => {
		...
		if (filterType !== "all") {
			filtered = filtered.filter(
				(user) => user.userType.toLowerCase() === filterType
			);
		}
						...
						<div className="charts-row">
							...
							<div className="chart-card">
								<div className="chart-header">
									...
									<span className="chart-subtitle">
										Platform usage throughout the day
									</span>
								</div>
								<div className="chart-wrapper">
								...
								{getFilteredUsers().length > 0 ? (
									getFilteredUsers().map((user, index) => (
										<tr key={user.id || index}>
											...
											<td>
												<span
													className={`profile-badge profile-${user.userType.toLowerCase()}`}
												>
													{user.userType}
												</span>
											</td>
												...
												<div className="engagement-score">
													<div className="score-bar">
														<div
															...
															style={{
																width: `${
																	(user.metrics.engagementScore || 0) * 100
																}%`,
															}}
														></div>
													</div>
											...
											<td>
												<span
													className={`completion-rate ${
														user.metrics.completionRate > 0.7
															? "high"
															: user.metrics.completionRate > 0.4
															? "medium"
															: "low"
													}`}
												>
					...
					>
						<div className="modal-content" onClick={(e) => e.stopPropagation()}>
							<div className="modal-header">
								<h3>
									User Insights: {selectedUser.name || selectedUser.email}
								</h3>
								<button
									className="close-btn"
									onClick={() => setSelectedUser(null)}
							...
							<div className="modal-body">
								<div className="user-detail-section">
									...
									<p>
										...
										<span
											className={`profile-badge profile-${selectedUser.userType.toLowerCase()}`}
										>
									...
									<div className="metrics-grid">
										<div className="metric-item">
											...
											<span className="metric-value">
												{selectedUser.metrics?.totalInteractions || 0}
											</span>
										</div>
										<div className="metric-item">
											<span className="metric-label">Workout Sessions</span>
											<span className="metric-value">
												{selectedUser.metrics?.workoutSessions || 0}
											</span>
										</div>
										<div className="metric-item">
											...
											<span className="metric-value">
												{Math.round(
													(selectedUser.metrics?.workoutCompletionRate || 0) *
														100
												)}
												%
											</span>
										...
										<div className="metric-item">
											...
											<span className="metric-value">
												{Math.round(
													(selectedUser.metrics?.consistencyScore || 0) * 100
												)}
												%
											</span>
										</div>
											...
											{selectedUser.topExercises.map((exercise, index) => (
												<li key={index}>
													...
													<span className="exercise-count">
														{exercise.interactionCount} interactions
													</span>
												</li>
											))}
										</ul>
								...
								<div className="user-detail-section">
									...
									{userDetailData.activityTimeline &&
								...
								<div className="user-detail-section">
									...
									{userDetailData.personalRecords &&
									userDetailData.personalRecords.length > 0 ? (
									...
									{userDetailData.workoutSummary?.allTime ? (
										<div className="summary-stats">
											...
											<div className="summary-stat">
												...
												<span className="stat-value">
													{userDetailData.workoutSummary.allTime.totalVolume ||
														0}
												</span>
											</div>
											<div className="summary-stat">
